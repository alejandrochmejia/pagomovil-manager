...
import { IconCamera, IconPhoto, IconPencil, IconAlertTriangle } from '@tabler/icons-react';
...
export default function ScanPage() {
  ...
  const [lastSource, setLastSource] = useState<CaptureSource>('camera');

  const scanConfigured = isScanConfigured();

  async function handleScan(source: CaptureSource = 'camera') {
    setLastSource(source);
    try {
      setState('capturing');
      const base64 = await captureReceipt(source);
    ...
    <div className="page">
      ...
      {state === 'idle' && (
        <div className={styles.options}>
          {perms.canScan && (
            <button type="button" className={styles.optionCard} onClick={() => handleScan('camera')} disabled={!scanConfigured}>
          ...
          {perms.canScan && (
            <button type="button" className={styles.optionCard} onClick={() => handleScan('gallery')} disabled={!scanConfigured}>
              <span className={styles.optionIcon}><IconPhoto size={32} stroke={1.5} /></span>
              <span className={styles.optionTitle}>Subir desde galería</span>
              <span className={styles.optionDesc}>
                Elige una imagen del comprobante y la IA extrae los datos
              </span>
            </button>
          )}
      ...
      {state === 'error' && (
        <div className={styles.center}>
          ...
          <div className={styles.errorActions}>
            ...
            <Button onClick={() => handleScan(lastSource)}>Intentar de nuevo</Button>
          </div>
        </div>
      )}
